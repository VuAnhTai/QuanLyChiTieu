...
void loadRate()
{
	...
	if (tongTien == 0){
		swprintf(buffer, MAX_CHAR, L"Ăn uống: 0%%");
		SetWindowText(rateAnUong, buffer);

		swprintf(buffer, MAX_CHAR, L"Di chuyển: 0%%");
		SetWindowText(rateDiChuyen, buffer);

		swprintf(buffer, MAX_CHAR, L"Nhà cửa: 0%%");
		SetWindowText(rateNhaCua, buffer);

		swprintf(buffer, MAX_CHAR, L"Xe cộ: 0%%");
		SetWindowText(rateXeCo, buffer);

		swprintf(buffer, MAX_CHAR, L"Nhu yếu phẩm: 0%%");
		SetWindowText(rateNhuYeuPham, buffer);

		swprintf(buffer, MAX_CHAR, L"Dịch vụ: 0%%");
